...
public class GoodsDAO {
   ...
   public List<GoodsVO> goodsListData(int page)
   {
	   ...
	   try
	   {
		   ...
		   // fno, 사진, 이름, 숫자
		   // no, name, sub, firtst_price, price, poster, hit
		   // *											   *
   ...
   public GoodsVO goodsDetailData(int no)
   {
	   ...
	   try
	   {
		   ...
		   ps.executeUpdate();
		   ////////////////////////////// 조회수 증가 
		   sql="SELECT no,hit,goods_discount,goods_name,goods_sub,goods_price,goods_first_price,goods_delivery,goods_poster "
			  +"FROM goods_all "
		      +"WHERE no=?";
		   ...
		   vo.setGoods_discount(rs.getInt(3));
		   ...
		   vo.setGoods_poster(rs.getString(9).replace("https", "http"));
		   // 메모리 닫기
		   rs.close();
	   }catch(Exception ex)
	   {
